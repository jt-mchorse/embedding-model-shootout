...
``prompt-regression-suite``, and ``rag-production-kit``.
...
README being updated fails this test loud — the same drift class that
``test_architecture_doc.py`` catches inside ``docs/architecture.md``,
but for the README's range citation.
"""
...
def test_decision_range_cites_latest_active() -> None:
    ...
    assert cited == latest, (
        ...
        f"D-{latest:03d}. Update the README's decision-range summary to "
        f"D-002…D-{latest:03d}."
    )
