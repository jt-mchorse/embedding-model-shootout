...
stall during `pip install`, infinite loop in an embedding-metric test,
stuck embedding-API call — therefore burns the full 6-hour ceiling before the
...
the next hop for `embedding-model-shootout` per issue #51. Policy band
[1, 30] matches; every job here lands at 15 since none of the current
workloads (pure-python embedding metric tests, ruff, memory-file existence
checks) approach that ceiling.

Spec / origin: this repo's #51.
"""
...
# (e.g., a future embedding-model benchmark suite that hits embedding APIs).
...
def test_job_timeout_in_policy_band(workflow: str, job_id: str, body: dict[str, Any]) -> None:
    ...
    assert MIN_TIMEOUT_MINUTES <= timeout <= MAX_TIMEOUT_MINUTES, (
        ...
        f"embedding-model benchmark job), bump MAX_TIMEOUT_MINUTES with a "
