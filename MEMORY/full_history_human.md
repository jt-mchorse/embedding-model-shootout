...
## 2026-07-07 — Issue #85: sweep exit 2 on structurally-malformed result JSON (~35 min)

**What got done.** `sweep aggregate` and `sweep plot` document a "name the bad file and exit 2" contract for hand-edited result files, but the `try/except` around `SweepResult.from_dict(...)` only caught `json.JSONDecodeError` and `OSError`. A result that is valid JSON but structurally malformed — a non-object, a missing required field, or a wrong-typed `recall_at_k`/`embed_latency_ms` container — escaped `from_dict` as a raw `KeyError`/`TypeError`/`AttributeError` at exit 1, bypassing the contract. `from_dict` is the single validation choke-point, but only its `__post_init__` numeric-range guards raised the clean `ValueError` the CLI translates to exit 2; structural failures happen before `__post_init__`. Fixed by making `from_dict` validate structure up front (isinstance-dict, required keys, container-typed `recall_at_k`/`embed_latency_ms`) and raise a `ValueError` naming the offending field, converting coercion `TypeError`s to `ValueError`; both sweep commands now catch `ValueError` alongside `JSONDecodeError`. Mirrors the sibling `_read_corpus_jsonl` loader and the #79/#83 external-result-file threat model. Added parametrized CLI exit-2 lock tests (five shapes × two commands) plus `from_dict` unit tests; kept the happy-path over-rejection guard. Full suite green, ruff clean. PR #86.

**Why prioritized.** Static issue queue was exhausted (lco#97 JT-gated, rest headless demo captures), so work came from parallel fresh-lens dogfood hunts. Four lens families were empty across the priority-tier repos (metric/ratio formulas, sort/ordering, threshold operators) and seven repos (CLI/config-default drift); only the exception-type/message-accuracy lens yielded this — a valid-JSON-but-malformed-object leak against a documented exit-code contract, distinct from the already-fixed `isinstance`-after-`json.loads` container-parity class (#110/#114/#150). Reproduced firsthand on clean main before filing and fixing.
...
## 2026-07-14 (night, issue #105) — embedder_name GFM cell escapes pipe but not newline (sibling of #79/#80)

`aggregate_markdown` escaped `|` in the free-form `embedder_name` cell but not `\n`/`\r`. A GFM row is a single physical line, so a newline in the name split one result across two physical lines and broke every row after it, corrupting the front-page `docs/benchmarks.md` table.
